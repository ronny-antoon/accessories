...
WindowAccessory::WindowAccessory(gpio_num_t button_down_pin, gpio_num_t button_up_pin, gpio_num_t motor_up_pin, gpio_num_t motor_down_pin, uint32_t time_to_open, uint32_t time_to_close)
{
    ...
    button_config_t button_down_config = {
    ...
    button_config_t button_up_config = {
        .type = BUTTON_TYPE_GPIO,
        .gpio_button_config = {
...
void WindowAccessory::identifyYourSelf()
{
    ESP_LOGI(__FILENAME__, "call %s", __FUNCTION__);
}
...
void WindowAccessory::moveBlindToTargetTask()
{
    ...
    while (!targetPositionReached(direction))
    {
        ...
        currentPosition += (direction == MoveDirection::UP) ? (double)m_checkInterval / (m_time_to_open * 1000) * 100
